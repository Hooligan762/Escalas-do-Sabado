...
export function PasswordManagementPage({
  ...
  const handleEditClick = (item: { id: string | number, name: string }, isUser: boolean = true) => {
    ...
    if (isUser) {
      ...
      const campusBasedUsername = item.name ? item.name.toLowerCase().replace(/[^a-z0-9]/g, '') : '';
  ...
  const handleSaveEdit = () => {
    ...
    if (existingUser) {
      ...
      if (onEditCampusAndUser && existingUser.campus) {
        const userCampusName = typeof existingUser.campus === 'object' ? existingUser.campus?.name : existingUser.campus;
        const campus = campusList.find(c => c.name === userCampusName);
        if (campus) {
          onEditCampusAndUser(campus.id, userCampusName || '', editingUsername.trim(), newPassword);
        }
      }
    } else {
  ...
  return (
    <div className="space-y-6" suppressHydrationWarning>
      ...
      <Card>
        ...
        <CardContent>
          <div className="border rounded-lg max-h-96 overflow-x-auto overflow-y-auto" suppressHydrationWarning>
            <Table className="min-w-full" suppressHydrationWarning>
              <TableHeader className="sticky top-0 bg-white z-10" suppressHydrationWarning>
                <TableRow suppressHydrationWarning>
                  <TableHead className="font-semibold min-w-[200px]">Nome/Campus</TableHead>
                  <TableHead className="font-semibold min-w-[120px]">Login</TableHead>
                  <TableHead className="font-semibold min-w-[100px]">Tipo</TableHead>
                  <TableHead className="font-semibold min-w-[150px]">Campus</TableHead>
                  <TableHead className="font-semibold min-w-[200px]">Senha</TableHead>
                  <TableHead className="font-semibold text-right min-w-[100px]">Ações</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  <TableRow key={`user-${user.id}`} className="hover:bg-gray-50">
                    {editingId === user.id ? (
                      ...
                      <>
                        ...
                        <TableCell className="font-mono text-blue-600">{user.username || 'N/A'}</TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 rounded-md text-xs font-medium ${
                            ...
                            {user.username === 'full' ? 'Super Admin' : user.role === 'admin' ? 'Admin' : 'Técnico'}
                          </span>
                        </TableCell>
                        <TableCell className="text-gray-600">{typeof user.campus === 'object' ? user.campus?.name : user.campus || 'Global'}</TableCell>
                        <TableCell>
