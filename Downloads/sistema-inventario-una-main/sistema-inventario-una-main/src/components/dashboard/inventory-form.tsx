...
import { PlusCircle, Save, X, Building2 } from "lucide-react";
...
export default function InventoryForm({ editingItem, onSave, onClear, user, activeCampus, categories, sectors, campusList }: InventoryFormProps) {
  ...
  React.useEffect(() => {
    if (editingItem) {
      ...
      const defaultValues = {
        ...
        obs: editingItem.obs || '',
          isFixed: !!editingItem.isFixed, // Garante booleano correto
      };
      ...
      Object.entries(defaultValues).forEach(([key, value]) => {
          if (key === 'isFixed') {
            form.setValue('isFixed', !!editingItem.isFixed);
          } else {
      ...
      if (user.role === 'admin') {
        ...
        // Técnico sempre usa seu próprio campus (extrair string do objeto)
        const userCampusName = typeof user.campus === 'object' ? user.campus?.name : user.campus;
        defaultCampus = userCampusName || '';
      }
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // Garantir que o campo isFixed seja booleano
    const itemData = {
      ...values,
  ...
  // Remover 'Administrador' e 'admin' dos campus disponíveis
  const campusNames = React.useMemo(() => campusList.map(c => c.name).filter(name => name && name.toLowerCase() !== 'administrador' && name.toLowerCase() !== 'admin'), [campusList]);
                  ...
                  <FormItem>
                    ...
                    {user.role === 'admin' ? (
                      ...
                      <FormControl>
                        <div className="flex items-center gap-2">
                          <Input
                            value={field.value || (typeof user.campus === 'object' ? user.campus?.name : user.campus)}
                            disabled={true}
                            className="bg-muted text-muted-foreground"
                            placeholder={(typeof user.campus === 'object' ? user.campus?.name : user.campus) || "Campus do técnico"}
                          />
                    ...
                    {user.role === 'tecnico' && (
                      <FormDescription className="text-xs text-blue-600">
                        Como técnico, você só pode cadastrar equipamentos no seu campus: <strong>{typeof user.campus === 'object' ? user.campus?.name : user.campus}</strong>
                      </FormDescription>
                    )}
                  </FormItem>
                )}
