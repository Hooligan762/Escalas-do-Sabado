...
type DisposalViewProps = {
    items: InventoryItem[];
    onDelete: (id: string | number, isDisposal: true) => void;
...
export default function DisposalView({ items = [], onDelete, onRestore }: DisposalViewProps) {
    ...
    const filteredItems = React.useMemo(() => {
        return items.filter(item =>
            ...
            (item.campus && item.campus.toLowerCase().includes(filter.toLowerCase()))
                    ...
                    <TableBody>
                        {paginatedItems.length > 0 ? paginatedItems.map(item => (
                            <TableRow key={item.id} className="bg-gray-50 dark:bg-gray-900/20">
                                ...
                                <TableCell>{format(typeof item.updated === 'string' ? parseISO(item.updated) : new Date(item.updated), 'dd/MM/yyyy')}</TableCell>
