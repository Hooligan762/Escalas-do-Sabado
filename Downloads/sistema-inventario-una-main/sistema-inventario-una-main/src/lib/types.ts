...
export type User = {
  ...
  name: string;
  role: 'admin' | 'tecnico';
  campus: string | { id: string | number; name: string }; // Permite string (admin) ou objeto (técnico)
...
export type Category = {
    id: string | number; // Changed to allow for numeric IDs from Postgres
    name: string;
    campus?: { id: string | number; name: string }; // Adicionado para sincronização com banco
}
