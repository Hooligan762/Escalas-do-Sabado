...
import { guardAgainstPhantomIds } from '../phantom-id-guard';
...
export async function updateInventoryItem(...args: Parameters<typeof db.updateInventoryItem>) {
  // 🛡️ PROTEÇÃO CRÍTICA - Bloquear IDs fantasma ANTES de chegar ao adapter
  const [id] = args;
  guardAgainstPhantomIds(id, 'updateInventoryItem [WRAPPER]');
...
export async function deleteInventoryItem(...args: Parameters<typeof db.deleteInventoryItem>) {
  // 🛡️ PROTEÇÃO CRÍTICA - Bloquear IDs fantasma ANTES de chegar ao adapter
  const [id] = args;
  guardAgainstPhantomIds(id, 'deleteInventoryItem [WRAPPER]');
...
export async function updateInventoryStatus(...args: Parameters<typeof db.updateInventoryStatus>) {
  // 🛡️ PROTEÇÃO CRÍTICA - Bloquear IDs fantasma ANTES de chegar ao adapter
  const [id] = args;
  guardAgainstPhantomIds(id, 'updateInventoryStatus [WRAPPER]');
