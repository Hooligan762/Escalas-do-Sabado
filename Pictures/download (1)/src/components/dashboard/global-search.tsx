...
export default function GlobalSearch({ inventory }: GlobalSearchProps) {
    ...
    React.useEffect(() => {
        if (debouncedSearchTerm.length > 2) {
            ...
            const filtered = inventory.filter(item => 
                item.patrimony?.toLowerCase().includes(lowerCaseTerm) ||
                item.serial.toLowerCase().includes(lowerCaseTerm) ||
                item.category.toLowerCase().includes(lowerCaseTerm) ||
                item.brand?.toLowerCase().includes(lowerCaseTerm) ||
                item.responsible?.toLowerCase().includes(lowerCaseTerm)
            );
            setResults(filtered);
    ...
    const handleSelectResult = (item: InventoryItem) => {
        setOpen(false);
        setSearchTerm('');
        // Here you would typically navigate to the item's page or open a detail modal
                        ...
                        <CommandList>
                            {results.length === 0 && searchTerm.length > 2 ? (
                                ...
                                {results.map(item => {
                                     ...
                                     return(
                                        <CommandItem key={item.id} onSelect={() => handleSelectResult(item)} value={item.id}>
